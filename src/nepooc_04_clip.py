...
Paste ALL of this as a single cell in Kaggle notebook nepooc_04_clip
GPU T4x2, Internet ON, all 3 datasets attached
Run first: !pip install git+https://github.com/openai/CLIP.git
"""
...
def push_results_to_github(token):
    ...
    os.system("git commit -m 'Auto: CLIP fine-tuned correct results'")
...
class CLIPFineTuned(nn.Module):
    def __init__(self, clip_model):
        ...
        self.classifier = nn.Sequential(
            nn.Linear(embed_dim * 2 + 1 + embed_dim, 512),
...
def run_clip(seed, data_fraction):
    print(f"\n{'='*55}\nCLIP | Seed={seed} | Fraction={data_fraction}\n{'='*55}")
    ...
    try:
        clip_params = list(model.module.clip.parameters())
        head_params = list(model.module.classifier.parameters())
    except:
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for batch in train_loader:
            ...
            with torch.amp.autocast('cuda'):
                logits = model(imgs, texts); loss = criterion(logits, labels)
    ...
    result = {
        'model': 'CLIP', 'seed': seed, 'fraction': data_fraction, 'split_type': 'random',
