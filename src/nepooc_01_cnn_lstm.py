...
- Weight decay: 1e-5
- Scheduler: StepLR (not Cosine)
- Class weights: 0.857 / 1.200 (inverse frequency)
- NO data augmentation
- NO label smoothing (removed)
- NO gradient clipping (removed)
- NO mixed precision (removed for methodology purity)
"""
...
BATCH_SIZE = 32           # ✅ Methodology Table VI: 32
EPOCHS     = 80           # ✅ Methodology Table VI: 80
LR         = 1e-4         # ✅ Methodology Table VI: 1e-4
WEIGHT_DECAY = 1e-5       # ✅ Methodology Table VI: 1e-5
STEP_SIZE  = 30           # StepLR step size
...
# ✅ Class weights: inverse frequency (Methodology Section IV-A)
n_total = len(df)
...
# ── Model: 5-layer CNN + BiLSTM (Methodology Section IV-E) ───────────────────
class CNN5Layer(nn.Module):
    def __init__(self):
        super().__init__()
        # ✅ CORRECTED: 64 → 128 → 256 → 512 → 512 (matching PDF)
        ...
        # Now 512 * 4 * 4 = 8192 → 512 projection
...
# ── Training runner (MATCHING METHODOLOGY) ────────────────────────────────────
def run_cnn_lstm(seed, data_fraction):
    ...
    # ✅ NO label smoothing (methodology doesn't mention it)
    criterion = nn.CrossEntropyLoss(weight=weights)
    # ✅ Adam optimizer (not AdamW)
    optimizer = torch.optim.Adam(model.parameters(), lr=LR, weight_decay=WEIGHT_DECAY)
    # ✅ StepLR scheduler (not Cosine)
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=STEP_SIZE, gamma=GAMMA)
    
    # ❌ NO gradient clipping
    # ❌ NO mixed precision (autocast/GradScaler)
    ...
    torch.save(best_state, f"results/model_CNN_LSTM_seed{seed}_frac{data_fraction}.pth")
